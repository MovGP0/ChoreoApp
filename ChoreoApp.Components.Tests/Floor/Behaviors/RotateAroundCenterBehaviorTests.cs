...
public sealed class RotateAroundCenterBehaviorTests(ITestOutputHelper testOutputHelper) : FeatureFixture
{
    ...
    [Scenario(DisplayName = "Should rotate selected positions around center")]
    public void RotateSelectedPositionsAroundCenter()
    {
        Runner.RunScenario(
            Given_a_rotate_around_center_context,
            Given_a_choreography_with_positions_is_loaded,
            Given_rotate_mode_is_active,
            When_the_user_selects_positions_with_rectangle,
            When_the_user_rotates_the_selection,
            Then_selected_positions_should_rotate,
            Then_cleanup_resources);
    }

    private void Given_a_rotate_around_center_context()
    {
        _context = FloorBehaviorTestContext<RotateAroundCenterBehavior>.Create(services =>
        {
    ...
    private void Given_rotate_mode_is_active()
    {
        _context.ShouldNotBeNull();
        _context.GlobalState.InteractionMode = InteractionMode.RotateAroundCenter;
        _context.StateMachine.TryApply(new RotateAroundCenterStartedTrigger()).ShouldBeTrue();
    }
    ...
    {
        _context.ShouldNotBeNull();
        _context.DragFromFloorTo(new Point(0, 2), new Point(1, 1));
    }

    private void Then_selected_positions_should_rotate()
    {
