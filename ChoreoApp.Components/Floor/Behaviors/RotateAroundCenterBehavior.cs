    ...
    IBehavior<FloorCanvasViewModel>
{
    ...
    {
        BehaviorLog.BehaviorActivated(logger, nameof(RotateAroundCenterBehavior), nameof(FloorCanvasViewModel));
    ...
    private void StartSelection(Point floorPoint)
    {
        ...
        stateMachine.TryApply(new RotateAroundCenterSelectionStartedTrigger());
    ...
    private void StartRotation(Point floorPoint)
    {
        ...
        _rotationCenter = CalculateCenter(globalState.SelectedPositions);
    ...
    {
        if (globalState.InteractionMode != Global.InteractionMode.RotateAroundCenter)
        {
            return false;
        }
